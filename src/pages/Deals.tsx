...
export default function Deals() {
  ...
  // Direct REST fetch fallback â€” uses the anon key directly instead of the
  ...
  // or status='published' set â€” many legit deals have status=null, status
  ...
  const fetchPropertiesViaRest = async (signal: AbortSignal): Promise<Property[] | null> => {
    ...
    // have workflow_stage='published' OR have is_published=true â€” this way
  ...
  const fetchDeals = async () => {
    ...
    console.log('[Deals] â–¶ fetchDeals starting. caller_type=', resolvedCallerType);
    ...
    const hardStopTimer = setTimeout(() => {
      console.warn('[Deals] â± Hard-stop timer (25s) fired â€” forcing loading=false');
    ...
    try {
      ...
      // most reliable path â€” no supabase-js client overhead, no cold-start
      ...
      try {
        ...
        if (restRows && restRows.length > 0) {
          ...
          sourceStrategy = 'REST';
          console.log(`[Deals] âœ“ Strategy 1 (REST) success: ${restRows.length} properties`);
        } else {
      ...
      if (!propertiesData || propertiesData.length === 0) {
        try {
          ...
          if (efError) {
            ...
            console.log(`[Deals] âœ“ Strategy 2 (edge function) success: ${efData.properties.length} properties`);
          } else {
      ...
      if (!propertiesData || propertiesData.length === 0) {
        try {
          ...
          if (!fallbackError && fallbackData?.length) {
            ...
            console.log(
              `[Deals] âœ“ Strategy 3 success: ${fallbackData.length} raw rows, ${visibleRows.length} after public-visibility filter`
            );
      ...
      // Always clear the hard-stop timer â€” we got past the fetch phase.
      clearTimeout(hardStopTimer);

      if (propertiesData && propertiesData.length > 0) {
        console.log(`[Deals] âœ… RESOLVED via ${sourceStrategy} with ${propertiesData.length} properties`);
        ...
        console.warn('[Deals] âŒ No properties found from any strategy â€” rendering empty state');
        setProperties([]);
        setLoading(false);
        setLastUpdated(new Date());
      }
        ...
        <div className="max-w-[1600px] mx-auto px-4 py-8">
          ...
          <div className="flex gap-6">
            ...
            <div className="flex-1 min-w-0">
              {/* Published Deals Counter â€” uses the server-side publish-filtered count
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-1.5">
                            ...
                            <span title={exactTimestamp}>
                              ...
                              {exactTimestamp && (
                                <span className="hidden sm:inline text-gray-400"> Â· {exactTimestamp}</span>
                              )}
                            </span>
                          </div>
                        </div>
