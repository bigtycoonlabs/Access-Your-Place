...
import { useSignatureRequests, isAwaitingSignature, SignDocumentDialog, type SignatureRequest } from './LandlordSignatures';
...
export default function LandlordPortalApplications({ landlordId, landlordName, onOpenChat }: Props) {
  ...
  const signatures = useSignatureRequests(landlordId);
  const [signing, setSigning] = useState<SignatureRequest | null>(null);
  const leaseToSign = (appId: string) =>
    signatures.requests.find((r) => r.corporate_application_id === appId && r.document_type === 'lease' && isAwaitingSignature(r));
  ...
  return (
    <div className="space-y-6">
                        ...
                        {(app.current_stage === 'approved' || app.current_stage === 'lease_generated') && (
                          <div className="bg-green-50 border border-green-200 rounded-lg p-4 mt-2">
                            ...
                            {leaseToSign(app.id) ? (
                              <div className="mt-3">
                                <button
                                  onClick={() => setSigning(leaseToSign(app.id) || null)}
                                  className="inline-flex items-center gap-2 bg-[#1a365d] text-white px-4 py-2.5 rounded-lg text-sm font-medium hover:bg-[#2d4a7c] transition-colors"
                                >
                                  <FileSignature className="w-4 h-4" /> Review and sign the lease
                                </button>
                              </div>
                            ) : !app.lease_url && (
                              <p className="text-sm text-green-600">The lease will appear here for you to sign once the team has prepared it.</p>
                            )}
                          </div>
                        )}
      ...
      <SignDocumentDialog
        ...
        onDone={() => { setSigning(null); signatures.reload(); fetchApplications(); }}
      />
    </div>
  );
}
