# investor-session Edge Function

A unified edge function that handles all investor authentication operations including registration, login, logout, and session management.

## Why Use This Function?

If the separate `investor-register` and `investor-login` functions are experiencing issues, this unified function provides an alternative approach that may avoid the "could not be cloned" error.

## Edge Function Code

Create file: `supabase/functions/investor-session/index.ts`
...
```bash
# Create function
supabase functions new investor-session

# Copy code to supabase/functions/investor-session/index.ts

# Deploy
supabase functions deploy investor-session --no-verify-jwt
```

## Frontend Integration

To use this unified function instead of separate register/login functions, update `InvestorLogin.tsx`:

### For Registration (around line 317):
```typescript
// Change from:
...
### For Login (around line 185):
```typescript
// The login already uses action parameter, just change the function name:
const { data, error } = await supabase.functions.invoke('investor-session', {
  body: { 
    action: 'login', 
...
|--------|----------------|-------------|
| `register` | email, password, full_name | Create new account |
| `login` | email, password | Authenticate user |
| `validate_session` | session_token | Check if session is valid |
| `refresh_session` | session_token | Extend session expiry |
| `logout` | session_token | End current session |
| `logout_all` | session_token | End all sessions |
| `update_profile` | session_token, + fields | Update investor profile |
| `verify_email` | verification_token | Verify email address |
