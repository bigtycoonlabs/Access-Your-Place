...
// Create a NEW staff member and send them their setup email (a personal invitation link
...
async function execTool(name: string, args: any, ctx: Ctx): Promise<unknown> {
  ...
  try {
    ...
    if (name === 'invite_staff') {
      ...
      if (args.confirmed !== true) {
        return { needs_confirmation: true, action: `create a staff account for ${first_name} ${last_name} (${email}) on the ${department.replace(/_/g, ' ')} team and email them their setup link`, instruction: 'Read back to the owner exactly who this will email — their name, their email, and their department — and get a clear yes. Explain that it emails the new teammate a personal invitation link where they choose their own password (no temporary password is sent), and that this email is all the gate they need (no second approval afterward). Then call again with confirmed:true.' };
      }
...
const TOOLS = [
  ...
  },
  {
    ...
    function: {
      name: 'record_closing',
      description: "Record a completed deal (a closing) into the company ledger and P&L, from details the staff member gives you or from an acquisition agreement they shared. This is a WRITE that changes the live P&L - only call with confirmed:true after you have read every number back and the staff member has clearly approved. deal_type is 'acquisition' (first-party: acquisition fee plus any setup fee) or 'third_party' (a resale: seller keeps 80%, the platform takes 20% of the acquisition listing cost). Never guess a number - if a figure is missing, ask for it.",
      parameters: {
        type: 'object',
        properties: {
...
STAFF INVITES (OWNERS ONLY): only the owners — Vission and Rel — can bring a new staff member onto the team. If an owner asks you to send a new teammate a staff login, add someone to the team, or invite a new hire, gather their first name, last name, email, and which department they'll join (Success Managers, Acquisition Managers, Setup Managers, Content Team, or Support Team). Then read it ALL back — who you'll email, their name, and their department — and get a clear yes before you do anything. Only after that clear yes, call invite_staff with confirmed:true. It creates their account and emails them a personal invitation link to the staff login, where they choose their own password (no temporary password is sent) — that email is all the gate the new teammate needs, so there is nothing more for the owner to approve afterward. If someone who is NOT an owner asks you to invite staff, tell them warmly that only an owner can do that (the tool refuses regardless). If the person already has a staff account, say so plainly instead of implying a new invite went out.
