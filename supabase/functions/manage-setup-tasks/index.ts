...
// ---- payment rails for client emails -----------------------------------------
// This function used to hardcode "Zelle to admin@cooperfamilyinc.net" into live fee
// emails quoting $2,500 and $3,350. That address is now only the bank fallback, so
// clients were handed outdated instructions attached to a real invoice.
//
// The fix is NOT to swap in the current tag. Hardcoding a destination is what created
// this bug, and it would go stale again the moment a rail changes. Worse, a destination
// typed into an email is permanent and forwardable: one wrong character sends a client's
// money somewhere unrecoverable, and the owners are blind and cannot catch a bad
// character by glancing at a screen.
//
// So the email NAMES the rails and points at the Payments tab, where PaymentMethodPanel
// renders each destination with a copy button straight from company_payment_methods.
// Nothing is transcribed, by a human or by us.
const PAYMENTS_TAB_URL = 'https://accessyourplace.com/investor/login';

// Reads only label / method_type / display_order. It deliberately NEVER selects
// `details`, which holds the tag, cashtag, wallet address and account numbers -- the
// values that must not leave the Payments panel.
async function activeRailLabels(supabase: any): Promise<string[]> {
  try {
    const { data, error } = await supabase
      .from('company_payment_methods')
      .select('label,method_type,display_order')
      .eq('is_active', true)
      .order('display_order', { ascending: true });
    if (error || !Array.isArray(data)) {
      console.error('manage-setup-tasks rail_read_failed', error?.message || 'no rows');
      return [];
    }
    return data.map((r: any) => r.label || r.method_type).filter(Boolean);
  } catch (e) {
    console.error('manage-setup-tasks rail_read_threw', e instanceof Error ? e.message : String(e));
    return [];
  }
}

// On a failed read this renders with no rail names rather than guessing one. An email
// saying "open the Payments tab" is always correct; one naming a retired rail is not.
function paymentBlockHTML(rails: string[] | undefined, accent: string): string {
  const named = Array.isArray(rails) && rails.length
    ? `We currently accept ${rails.length > 1 ? rails.slice(0, -1).join(', ') + ' and ' + rails[rails.length - 1] : rails[0]}. `
    : '';
  return `<div class="payment-info"><strong>Payment:</strong> ${named}For your security we never put account details in an email. Sign in and open your <strong>Payments</strong> tab &mdash; every method there has a copy button with the exact details, taken straight from our records. <a href="${PAYMENTS_TAB_URL}" style="color:${accent};font-weight:600">Open the Payments tab</a>.<br><br><span style="font-size:13px;color:#666">Please note: wires are received by <strong>Cooper Family Inc</strong>, the parent company of Set Up Your Place LLC, which operates Access Your Place. That is the name you will see on the transfer.</span></div>`;
}

function paymentBlockText(rails: string[] | undefined): string {
  const named = Array.isArray(rails) && rails.length ? `We currently accept ${rails.join(', ')}. ` : '';
  return `\n\nPayment: ${named}For your security we never put account details in an email. Sign in at ${PAYMENTS_TAB_URL} and open your Payments tab; every method there has a copy button with the exact details from our records.\n\nPlease note: wires are received by Cooper Family Inc, the parent company of Set Up Your Place LLC, which operates Access Your Place. That is the name you will see on the transfer.`;
}
  ...
  try {
    ...
    if (mgr?.email && rk) {
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({ from: 'Penny <penny@accessyourplace.com>', reply_to: ['success@accessyourplace.com'], to: [mgr.email], subject, html })
      });
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'email_pro_portal_link') {
      ...
      await fetch('https://api.resend.com/emails', { method: 'POST', headers: { 'Authorization': `Bearer ${rk}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: 'Penny <penny@accessyourplace.com>', reply_to: ['success@accessyourplace.com'], to: [project.setup_pro_email], subject: `Your Setup Pro Portal: ${project.property_address}`, html: `<h2>Setup Pro Portal</h2><p>Hi ${project.setup_pro_name || 'there'},</p><p>Here is your portal link for <strong>${project.property_address}</strong>:</p><p><a href="${portalUrl}" style="display:inline-block;background:#1a365d;color:#fff;padding:14px 28px;border-radius:8px;text-decoration:none;font-weight:600">Open Pro Portal</a></p><p>This link is unique to you.</p>` }) });
    ...
    if (action === 'preview_recap') {
      ...
      (vars as any).payment_rails = await activeRailLabels(supabase);
