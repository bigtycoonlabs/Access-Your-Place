    ...
    emailBody = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background: #1a365d; padding: 15px; text-align: center;">
          <p style="color: #d4a574; margin: 0; font-size: 12px;">&copy; 2026 Access Your Place. All rights reserved.</p>
        </div>
      </div>
    `;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    { const denied = await gate(req, body, String(body?.action || ''), corsHeaders, {"clientActions": ["add_message", "create", "get_investor_disputes"]});
