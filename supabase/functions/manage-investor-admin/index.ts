...
// manage-investor-admin v17.3 - Fixed: createNotification uses 'data' column (not 'metadata') for investor_notifications
const corsHeaders = {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('[v17.3] Action:', action);
    ...
    async function fetchInvestors(extraParams = '') {
      ...
      if (!res.ok) { const t = await res.text().catch(() => ''); console.error('[v17.3] Error:', t.substring(0, 300)); return []; }
    ...
    // v17.3 FIX: Changed 'metadata' to 'data' - the investor_notifications table uses a 'data' column (jsonb), not 'metadata'
    ...
    // ===== SYNC PORTFOLIO COUNTS =====
    if (action === 'sync_portfolio_counts') {
      console.log('[v17.3] Syncing portfolio counts for all investors...');
      ...
      console.log('[v17.3] Portfolio sync complete. Updated:', updated, 'Errors:', errors);
    ...
    // ===== GET ASSIGNED INVESTORS (for AM tab) =====
    ...
    // ===== ADD PROPERTY TO PORTFOLIO (manual add) =====
    if (action === 'add_property_to_portfolio' || action === 'add_portfolio_property') {
      ...
      if (!insertRes.ok) { const errText = await insertRes.text(); console.error('[v17.3] Portfolio insert failed:', errText); return json({ success: false, error: `Failed to add property: ${errText}` }); }
    ...
    // ===== ADD DEALFLOW PROPERTY TO PORTFOLIO =====
    if (action === 'add_dealflow_to_portfolio') {
      ...
      const insertRes = await fetch(`${SUPABASE_URL}/rest/v1/investor_portfolio`, { method: 'POST', headers: mutH, body: JSON.stringify(portfolioData) });
      if (!insertRes.ok) { const errText = await insertRes.text(); console.error('[v17.3] Dealflow portfolio insert failed:', errText); throw new Error(`Failed: ${errText}`); }
      const newProp = (await insertRes.json())?.[0];
    ...
    console.error('[v17.3] Error:', error.message);
