...
serve(async (req) => {
  ...
  try {
    ...
    const { action, user_id: claimedUserId, user_name, message, session_id, conversation_history } = body
    const user_id = await verifiedUserId(req, Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '', String(claimedUserId || ''), 'landlord')
    if (claimedUserId && !user_id) console.log('penny-landlord-chat unverified_user_id_dropped')
