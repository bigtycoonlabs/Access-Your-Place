...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const denied = await gate(req, body, String(body?.action || ''), corsHeaders, { publicActions: [], clientActions: ['get_documents', 'mark_viewed', 'sign_document'], ownRow: {mark_viewed: { table: 'document_signatures', field: 'document_id' }, sign_document: { table: 'document_signatures', field: 'document_id' }} });
