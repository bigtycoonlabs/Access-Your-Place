...
Deno.serve(async (req) => {
  ...
  try {
    ...
    { const denied = await gate(req, __body, String(__body?.action || ''), corsHeaders, {});
      if (denied) return denied; }
    const { action, inquiryId, newStatus, propertyAddress, investor_id, property_data, acquisition_data, referral_data, report_data } = __body;
