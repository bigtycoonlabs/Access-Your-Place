    ...
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 640px; margin: 0 auto; background: #f9fafb;">
      ...
      <div style="background: #f3f4f6; padding: 20px 24px; text-align: center; border-top: 1px solid #e5e7eb;">
        <p style="margin: 0 0 8px 0; color: #6b7280; font-size: 14px;">
          Penny AI — Your Deal Alert Assistant at Access Your Place
        </p>
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    { const denied = await gate(req, body, String(body?.action || ''), corsHeaders, {"clientActions": ["get_alert_history", "get_alert_settings", "preview_matches", "save_alert_settings"]});
      ...
      for (const pref of allPrefs) {
        ...
        if (frequency === 'instant') {
          ...
          if (pref.email_alerts && investor.email) {
            ...
            const subject = `New Deal Alert: ${property.city}, ${property.state} — ${score}% Match`;
          ...
          if (pref.push_alerts) {
            ...
            const pushBody = `${property.bedrooms}BR/${property.bathrooms}BA ${property.operation_type || 'property'} — $${property.monthly_rent?.toLocaleString() || 'TBD'}/mo`;
    ...
    if (action === 'send_digest') {
      ...
      for (const pref of prefsWithDigest) {
        ...
        if (pref.email_alerts && investor.email) {
          ...
          const subject = `Your ${frequency === 'daily' ? 'Daily' : 'Weekly'} Deal Digest — ${deals.length} New Match${deals.length > 1 ? 'es' : ''}`;
          const sent = await sendEmail(investor.email, subject, emailHtml);
          if (sent) digestsSent++;
        }
        ...
        if (frequency === 'instant') {
          ...
          if (pref.email_alerts && investor.email) {
            ...
            const subject = deals.length === 1
              ? `New Deal Alert: ${deals[0].property.city}, ${deals[0].property.state} — ${deals[0].score}% Match`
          ...
          if (pref.push_alerts) {
            ...
            const pushBody = deals.length === 1
              ? `${deals[0].property.bedrooms}BR/${deals[0].property.bathrooms}BA — $${deals[0].property.monthly_rent?.toLocaleString() || 'TBD'}/mo`
