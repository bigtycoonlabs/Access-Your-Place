...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // â”€â”€ Parse optional body params â”€â”€
    let forceRun = false;
    try {
      const body = await req.json();
      forceRun = body?.force === true;
    } catch { /* no body â€” fine */ }

    // â”€â”€ Idempotency guard: skip if already ran within last 14 minutes â”€â”€
    if (!forceRun) {
      ...
      if (recentRun && recentRun.length > 0) {
        console.log(`[check-error-thresholds] Skipping â€” already ran at ${recentRun[0].started_at}`);
    ...
    // â”€â”€ Check 1: More than 10 errors of the same type in 15 minutes â”€â”€
    ...
    // â”€â”€ Check 2: 100% failure rate for a specific function in the last hour â”€â”€
    ...
    // â”€â”€ Check 3: Any auth-type errors â”€â”€
    ...
    // â”€â”€ Compute comparison badge data (current 1h vs previous 1h) â”€â”€
    ...
    // â”€â”€ Compute hourly trend data (last 12 hours) â”€â”€
    ...
    // â”€â”€ Send email alerts for new alerts â”€â”€
    let emailsSent = 0;
    ...
    if (newAlerts.length > 0 && resendApiKey) {
      ...
      if (recipientEmails.length > 0) {
          ...
          try {
            const emailResponse = await fetch('https://api.resend.com/emails', {
              ...
              body: JSON.stringify({
                ...
                to: recipientEmails,
                subject: `[${severityLabel}] ${alert.title} | ${trendDirection === 'up' ? 'â–²' : trendDirection === 'down' ? 'â–¼' : 'â€”'} ${Math.abs(changePct)}% vs prev hour`,
                html: emailHtml,
              }),
            });
        ...
        console.log('No recipient emails found â€” logging alerts without sending');
        for (const alert of newAlerts) {
          await supabase.from('error_alert_log').insert({
            alert_type: alert.type,
            alert_key: alert.alert_key,
    ...
    // â”€â”€ Log execution completion â”€â”€
    if (execId) {
      await supabase
        .from('cron_execution_log')
        .update({
