...
Deno.serve(async (req) => {
  ...
  try {
    ...
    { const denied = await gate(req, __body, String(__body?.action || ''), corsHeaders, {"clientActions": ["get_match_history", "get_preferences", "save_preferences"]});
      if (denied) return denied; }
    const { action, investor_id, property_id, preferences } = __body;
