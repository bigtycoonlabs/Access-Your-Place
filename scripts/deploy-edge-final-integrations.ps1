$Manifest = "C:\WebsiteMigrations\accessyourplace-code\scripts\final-integration-functions.txt"
...
Write-Host ""
Write-Host "Final integration deploy complete."

if ($Failed.Count -gt 0) {
  Write-Host "Failed functions:" -ForegroundColor Red
  $Failed | ForEach-Object { Write-Host $_ -ForegroundColor Red }
} else {
  Write-Host "All final integration functions deployed successfully." -ForegroundColor Green
}