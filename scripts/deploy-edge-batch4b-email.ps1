$Manifest = "C:\WebsiteMigrations\accessyourplace-code\scripts\batch4b-email-functions.txt"
...
Write-Host ""
Write-Host "Batch 4B email deploy complete."

if ($Failed.Count -gt 0) {
  Write-Host "Failed functions:" -ForegroundColor Red
  $Failed | ForEach-Object { Write-Host $_ -ForegroundColor Red }
} else {
  Write-Host "All Batch 4B email functions deployed successfully." -ForegroundColor Green
}